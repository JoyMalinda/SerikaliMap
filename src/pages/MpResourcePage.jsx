...
export default function MpResource() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get("/officials/mps");
        setData(res.data);
  ...
  const filteredLeaders = officials.filter((leader) => {
    ...
    return matchesName && matchesParty && matchesCounty;
  });
      ...
      <option key={idx} value={party}>
        {party}
          ...
          <div className="p-4 dark:bg-gray-800 rounded-lg shadow">
            <div className="bg-white dark:bg-gray-800 shadow overflow-x-auto h-120">
                ...
                <tbody>
                  {filteredLeaders.map((leader, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-1">Hon. {toTitleCase(leader.name)}</td>
                      <td className="px-4 py-1">{leader.constituency}</td>
          ...
          {/* Gender & Party Distribution Charts */}
<h3 className="text-xl font-semibold dark:text-white mt-8 mb-4">
  Statistics Overview
</h3>

<div className="flex flex-col md:flex-row gap-6">
  {/* Gender Distribution */}
  <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow w-full md:w-1/2">
    <h4 className="text-lg font-medium mb-2 dark:text-gray-200">
      Gender Distribution (All MPs)
    </h4>
    <ResponsiveContainer width="100%" height={300}>
      <PieChart>
        <Pie
          data={[
            { name: "Male", value: stats.gender_counts.all_mps.male },
            { name: "Female", value: stats.gender_counts.all_mps.female },
          ]}
          dataKey="value"
          nameKey="name"
          outerRadius={100}
          label
        >
          <Cell fill="#377eb8" /> {/* Blue for Male */}
          <Cell fill="#d32240" /> {/* Red for Female */}
        </Pie>
        <Legend />
        <Tooltip />
      </PieChart>
  ...
  <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow w-full md:w-1/2">
    <h4 className="text-lg font-medium mb-2 dark:text-gray-200">
      Party Distribution (Elected Only)
    </h4>
    <ResponsiveContainer width="100%" height={300}>
      <PieChart>
        <Pie
          data={stats.party_distribution.elected_only.map((p) => ({
            name: p.abbrev,
            value: p.count,
          ...
          label={false}
        >
          {stats.party_distribution.elected_only.map((p, idx) => (
            <Cell key={idx} fill={getPartyColor(p.abbrev, p.name)}/>
          ))}
        </Pie>
        <Tooltip
